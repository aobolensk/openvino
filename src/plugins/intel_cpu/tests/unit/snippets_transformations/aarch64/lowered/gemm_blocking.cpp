...
TEST_F(GemmCPUBlockingTest, Floating) {
    ...
    }
    {
        ...
        init_expr_descriptors(gemm_expr, {{m_blk, k_blk}, {k_blk, n_blk}, {m_blk, n_blk}}, {layout_a, layout_b, layout_c});
        create_gemm_loop_infos(linear_ir_ref, gemm_expr, 384, m_blk, 1024, k_blk, 384, n_blk);
        gemm_expr->set_loop_ids({2, 1, 0});
...
TEST_F(GemmCPUBlockingTest, Floating_LargeK) {
    ...
    }
    {
        ...
        auto gemm = linear_ir_ref->push_node<aarch64::GemmCPU>(data_a.second, data_b.second,
                                                               PortDescriptor{}, PortDescriptor{}, PortDescriptor{});
        const auto& gemm_expr = *gemm.first;
        init_expr_descriptors(gemm_expr, {{m_blk, k_blk}, {k_blk, n_blk}, {m_blk, n_blk}});
        create_gemm_loop_infos(linear_ir_ref, gemm_expr, m, m_blk, k, k_blk, n, n_blk);
        gemm_expr->set_loop_ids({2, 1, 0});
        auto result = linear_ir_ref->push_node<ov::opset10::Result>(gemm.second);
    }
}
...
// NOTE: Quantized_FC and BFloat16 tests are not included for AArch64 as GemmCPU
// currently only supports f32 precision on AArch64 architecture
