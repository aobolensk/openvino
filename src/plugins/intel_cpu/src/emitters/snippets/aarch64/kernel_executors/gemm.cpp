...
void GemmKaiKernelExecutor::execute(const GemmKaiKernelExecutor* executor, void* in0, void* in1, void* out0) {
    ...
    fflush(stdout);
    try {
        test_read = *static_cast<char*>(in0);
    ...
    try {
        test_read = *static_cast<char*>(in1);
    ...
    try {
        test_read = *static_cast<char*>(out0);
    ...
    // EXPERIMENT 1: Check if in1 and out0 alias (corruption source)
    if (in1 == out0) {
        printf("DEBUG: CRITICAL - in1 and out0 are aliased! This will corrupt packed RHS data.\n");
    }
    ...
    // EXPERIMENT 3: Calculate and print packed RHS buffer size requirements
    size_t max_rhs_packed_offset = (N - 1) * (K + 1) * sizeof(float);
    size_t total_rhs_buffer_needed = N * (K + 1) * sizeof(float);
    printf("DEBUG: BUFFER SIZE ANALYSIS:\n");
    printf("DEBUG:   Max rhs_packed_offset expected: %zu bytes\n", max_rhs_packed_offset);
    printf("DEBUG:   Total RHS buffer needed: %zu bytes\n", total_rhs_buffer_needed);
    printf("DEBUG:   Largest access will be at offset %zu\n", max_rhs_packed_offset);
